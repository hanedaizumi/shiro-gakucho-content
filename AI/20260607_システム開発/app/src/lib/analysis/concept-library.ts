...
> = {
  ...
  "ATRを使った損切り・利確の科学": {
    analogy:
      "天気予報の「波の高さ」と同じです。波が3メートルの日に、堤防を1メートルにしたら普通に越えられますよね。ATRは「BTCの1日の平均的な波の高さ」。損切りはこの波より外側に置かないと、ただのノイズで狩られます。",
    ngAction:
      "「お金が減るのが怖いから」と損切りを浅く（数百ドルとか）置くこと。ATRより内側の損切りは、方向が合っていても普通の値動きで狩られて終わります。損切り貧乏の正体はほぼこれです。",
    commentPrompt:
      "損切り、毎回同じ幅で置いてますか？固定派・相場に合わせる派をコメントで教えてください。",
  },
  "出来高で見抜く本物のブレイク": {
    analogy:
      "お店の行列と同じです。「開店しました！」の声（ブレイク）だけでは本物か分からない。実際にお客さん（出来高）が押し寄せているかを見れば、人気店か閑古鳥かは一目瞭然です。出来高の伴わないブレイクは、声だけの閑古鳥です。",
    ngAction:
      "ラインを抜けた瞬間に出来高を確認せず飛び乗ること。出来高が平均以下のブレイクは、大口が個人を釣るための「ダマシ」の可能性が高い。ブレイク時の出来高が直近平均の1.5倍以上あるかを必ず確認してください。",
    commentPrompt:
      "ダマシのブレイク、引っかかったことありますか？ある・ないをコメントで正直に教えてください。",
  },
  "ローソク足のヒゲが語る攻防（ピンバー）": {
    analogy:
      "腕相撲の腕の傾きと同じです。一度大きく傾いた（長いヒゲ）のに元に戻ったということは、押し込まれた側が押し返したという証拠。長い下ヒゲは「売りが全力を出したのに買いが勝った」跡なんです。",
    ngAction:
      "ヒゲ1本だけを見て即エントリーすること。ピンバーは「どこで出たか」が9割です。何もないところで出たヒゲはただのノイズ。重要なサポート・レジスタンス上で出た時だけ意味を持ちます。",
    commentPrompt:
      "ローソク足、実体派ですか？ヒゲまで見る派ですか？コメントで教えてください。",
  },
};
...
export function buildConceptFourStep(
  ...
  > = {
    ...
    "ATRを使った損切り・利確の科学": {
      definition:
        "ATR（Average True Range）とは「BTCが1日に平均でどれくらい動くか」を数値化した指標です。TradingViewで「ATR」と検索すれば無料で表示できます。今日から損切り・利確の幅を「感覚」ではなく「数字」で決められるようになります。",
      chartApplication: `現在のATR(14日)は${formatPrice(t.atr14)}ドル。つまりBTCは今、1日に平均${formatPrice(t.atr14)}ドル動いています。これより浅い損切りは「普通の値動き」で狩られます。例えば${support ? formatPrice(support.price) : formatPrice(t.currentPrice * 0.97)}ドルのサポートでロングするなら、損切りはラインからATR×0.5（約${formatPrice(Math.round(t.atr14 * 0.5))}ドル）外側が目安です。`,
      benefit:
        "「損切りが浅すぎて狩られる→悔しくて再エントリー→また狩られる」という負の連鎖が止まります。ボラティリティに合わせて損切り幅が自動で決まるので、相場が荒れても冷静に対応できます。",
      entryBridge: `だから今週は、エントリーポイントだけでなく「ラインからATR×0.5外側」の損切り位置をセットで決めてから入ります。具体的な数字はこの後のシナリオで提示します。`,
    },
    "出来高で見抜く本物のブレイク": {
      definition:
        "ラインを抜けた時に「出来高が伴っているか」でブレイクの真偽を判定する手法です。本物のブレイクは大口の資金が入るため出来高が急増します。出来高が普段と変わらないブレイクは、ダマシの可能性が高いです。",
      chartApplication: `${t.volumeSpike ? "直近で出来高の急増が確認されており、今の値動きには大口の資金が伴っています。" : "直近の出来高は平均水準で、ここからのブレイクは出来高の確認が必須の局面です。"}${support ? `${formatPrice(support.price)}ドルの攻防では、割れた時の出来高が直近平均の1.5倍以上あるかを見てください。` : ""}出来高が伴わないままラインを抜けた場合、数時間で元の位置に戻る「ダマシ」を警戒します。`,
      benefit:
        "ブレイクアウトの勝率が大きく変わります。大口が個人を釣るためのダマシブレイクを事前に見分けられるため、「抜けた瞬間に飛びついて高値掴み」という最悪のパターンを回避できます。",
      entryBridge: `だから今週は重要ラインのブレイク時に必ず出来高を確認し、平均1.5倍以上＋ローソク足実体での突破が揃った時だけエントリーします。`,
    },
    "ローソク足のヒゲが語る攻防（ピンバー）": {
      definition:
        "ローソク足の「ヒゲ」は売りと買いの攻防の跡です。特に実体が小さく片側のヒゲが長い「ピンバー」は、一度押し込まれた側が押し返した強い反転サインになります。インジケーター不要、ローソク足だけで今日から使えます。",
      chartApplication: `${t.candleCharacteristics.includes("下ヒゲ") ? `直近の日足には下ヒゲが出ており、下値で買いが入った形跡があります。${support ? `${formatPrice(support.price)}ドルのサポート上でこの形が続くかが焦点です。` : ""}` : t.candleCharacteristics.includes("上ヒゲ") ? `直近の日足には上ヒゲが出ており、上値で売り圧力が出ています。${resistance ? `${formatPrice(resistance.price)}ドルのレジスタンス付近では特に注意が必要です。` : ""}` : `現在は明確なピンバーが出ていない状態です。${support ? `${formatPrice(support.price)}ドルのサポート` : "重要ライン"}にタッチした時のヒゲの出方が今週の焦点になります。`}`,
      benefit:
        "インジケーターが一切なくても、ローソク足1本で「その価格帯の売り買いの力関係」が読めるようになります。ラインとセットで使えば、反発を確認してから入る習慣が身につき、飛びつきエントリーが激減します。",
      entryBridge: `だから今週は${support ? formatPrice(support.price) : formatPrice(t.currentPrice * 0.97)}ドル付近で長い下ヒゲ＋4時間足陽線確定が出るかに注目し、それをロングのトリガーとします。`,
    },
  };
